...
describe Lulu::ApiClient do
  context 'initialization' do
    context 'URL stuff' do
      context 'host' do
        it 'removes http from host' do
          Lulu.configure { |c| c.host = 'http://example.com' }
          expect(Lulu::Configuration.default.host).to eq('example.com')
        end

        it 'removes https from host' do
          Lulu.configure { |c| c.host = 'https://wookiee.com' }
          expect(Lulu::ApiClient.default.config.host).to eq('wookiee.com')
        end

        it 'removes trailing path from host' do
          Lulu.configure { |c| c.host = 'hobo.com/v4' }
          expect(Lulu::Configuration.default.host).to eq('hobo.com')
        end
      end

      context 'base_path' do
        it "prepends a slash to base_path" do
          Lulu.configure { |c| c.base_path = 'v4/dog' }
          expect(Lulu::Configuration.default.base_path).to eq('/v4/dog')
        end
        ...
        it "ends up as a blank string if nil" do
          Lulu.configure { |c| c.base_path = nil }
          expect(Lulu::Configuration.default.base_path).to eq('')
        end
      end
    end
  end
  ...
  describe 'params_encoding in #build_request' do
    let(:config) { Lulu::Configuration.new }
    let(:api_client) { Lulu::ApiClient.new(config) }

    it 'defaults to nil' do
      expect(Lulu::Configuration.default.params_encoding).to eq(nil)
  ...
  describe 'timeout in #build_request' do
    ...
    it 'defaults to 0' do
      expect(Lulu::Configuration.default.timeout).to eq(0)
  ...
  describe '#build_collection_param' do
    let(:param) { ['aa', 'bb', 'cc'] }
    let(:api_client) { Lulu::ApiClient.new }
